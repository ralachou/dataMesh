...
- Explainability panel (cards + chart + tables) with reconciliation delta
- Clickable breadcrumbs for business + risk-factor drill paths
- Uses Polars for filtering/aggregation where feasible (pandas only as Excel reader fallback)
"""
...
def load_data(xlsx_path: Path) -> Dict[str, pl.DataFrame]:
    ...
    # Base fact table used only for TOP granular riskFactors slices (never shipped wholesale)
...
VALUE_FORMATTER_MM = {
    "function": "return (params.value === null || params.value === undefined) ? '' : params.value.toLocaleString(undefined,{minimumFractionDigits:1, maximumFractionDigits:1});"
}
VALUE_FORMATTER_PCT = {
    "function": "return (params.value === null || params.value === undefined) ? '' : params.value.toLocaleString(undefined,{minimumFractionDigits:1, maximumFractionDigits:1});"
}
...
init_rf_rows, _ = rf_grid_rows(INIT_BIZ_PATH, DEFAULT_RF_DEPTH, INIT_RF_PATH)
...
def on_business_controls(depth: int, view_mode: str, reset_clicks: int, current_path: List[str]):
    ...
    return rows, col_defs, grid_opts, biz_path, rf_path
...
# =========================
# CALLBACK 2: Business selection -> update business store + clear rf store + rebuild rf grid + explain
# =========================
@app.callback(
...
def on_business_selected(
    ...
    return biz_path, rf_path, rf_rows, rf_col_defs, rf_grid_opts, cards, bd_children, top_children, fig


# =========================
# CALLBACK 3: RF selection -> update rf store + refresh cards + top RF table + chart
# =========================
@app.callback(
    Output("store-rf-path", "data", allow_duplicate=True),
...
def on_rf_selected(
    ...
    biz_total = get_business_node_total(biz_path)
    rf_contrib = rf_selected_contribution(biz_path, rf_path, int(rf_depth))

    rf_label = " / ".join(rf_path) if rf_path else "All RFs"
    delta = biz_total - rf_contrib_total_within_business(biz_path, int(rf_depth))

    only_neg = "neg" in (only_negs or [])
    top_df = top_risk_factors(biz_path, rf_path, int(topn or DEFAULT_TOPN), only_neg)
